...
void init_reductions() {
    ...
    for (int depth = 1; depth < MAX_PLY; depth++) {
        for (int moves = 1; moves < MAX_MOVES; moves++)
            reductions[depth][moves] = 1 + log(depth) * log(moves) / (LMR_BASE * 0.01);
    }
}
...
Score Search::absearch(int depth, Score alpha, Score beta, Stack *ss) {
     ...
     * Razoring
     *******************/
    if (depth < RAZOR_MAX_DEPTH && ss->eval + RAZOR_MARGIN < alpha) return qsearch<NONPV>(alpha, beta, ss);

    /********************
     * Reverse futility pruning
     *******************/
    if (std::abs(beta) < VALUE_TB_WIN_IN_MAX_PLY)
        if (depth < RFP_MAX_DEPTH && ss->eval - RFP_MARGIN * depth + RFP_IMPROVING * improving >= beta) return beta;
    ...
    if (board.nonPawnMat(color)
        ...
        && ss->eval >= beta) {
        // clang-format on
        int R = NMP_BASE_R + std::min(4, depth / NMP_DEPTH_DIV) + std::min(3, (ss->eval - beta) / NMP_EVAL_DIV);
    ...
    while ((move = mp.nextMove()) != NO_MOVE) {
        ...
        if (!root_node && best > VALUE_TB_LOSS_IN_MAX_PLY) {
            ...
            if (capture) {
                // SEE pruning
                if (depth < 6
                    && !see::see(board, move, -(depth * SEE_QUIET_MULT)))
                    continue;
            } else {
                ...
                if (!in_check
                    ...
                    && typeOf(move) != PROMOTION
                    && depth <= 5
                    && quiet_count > (LMP_BASE + depth * depth))

                    continue;
                // SEE pruning
                if (depth < 7
                    && !see::see(board, move, -(depth * SEE_CAPT_MULT)))
                    continue;
            }
            // clang-format on
        }
